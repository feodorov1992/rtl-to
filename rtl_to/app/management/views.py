...
class OrderEditView(PermissionRequiredMixin, View):
    ...
    def get(self, request, pk):
        ...
        order_form.fields['manager'].queryset = User.objects.filter(user_type='manager', is_superuser=False)
...
class ExtOrderListView(LoginRequiredMixin, FilteredListView):
    ...
    def form_valid(self, form):
        ...
        if form.cleaned_data['from_date']:
            queryset = queryset.filter(date__gte=form.cleaned_data['from_date'])
        if form.cleaned_data['to_date']:
            queryset = queryset.filter(date__lte=form.cleaned_data['to_date'])
        return queryset
